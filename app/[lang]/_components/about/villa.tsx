...
export default function AboutVilla({ dict }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div className='relative z-10 mx-auto flex h-full max-h-[95vh] min-h-[640px] max-w-screen-xl flex-col px-4 py-4 pt-16 pb-24 sm:py-6 md:py-12 md:pb-40'>
        {/* Desktop: Small Tagline top right */}
        <div className='hidden md:block'>
          <div
            className={clsx(
              'mb-8 flex justify-end duration-1000 ease-out motion-safe:transition-all',
              isVisible
          ...
          >
            <Image
              ...
              loading='lazy'
              height={220}
              className='h-auto w-[280px] object-contain drop-shadow duration-300 hover:scale-105 motion-safe:transition-transform lg:w-[320px]'
        ...
        <div className='flex flex-1 flex-col overflow-y-auto'>
          ...
          <div className='flex items-start justify-between gap-4 md:block'>
            <h2
              id='villa-title'
              className='text-2xl font-extrabold tracking-wide text-white uppercase drop-shadow sm:text-3xl md:text-4xl'
            >
          ...
          <div
            className={clsx(
              'mt-6 space-y-4 text-white drop-shadow duration-1000 ease-out motion-safe:transition-all',
              isVisible
                ? 'translate-y-0 opacity-100'
                : 'translate-y-8 opacity-0'
            )}
          ...
          >
            {PARAGRAPHS.map((key, index) => (
              <p
                key={key}
                className={clsx(
                  'text-sm leading-relaxed duration-700 ease-out motion-safe:transition-all sm:text-base',
        ...
        <div
          className={clsx(
            'mt-8 hidden items-end justify-between duration-1000 ease-out motion-safe:transition-all md:flex',
...
function EnhancedButton({ href, ariaLabel, children }: EnhancedButtonProps) {
  return (
    <Link
      ...
      className='group relative inline-flex shrink-0 items-center gap-2 overflow-hidden rounded-full bg-white px-4 py-2 text-sm font-bold text-sky-700 shadow-lg ring-1 ring-black/10 duration-300 hover:scale-105 hover:bg-sky-700 hover:text-white hover:shadow-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-white/80 motion-safe:transition-all sm:text-base'
      aria-label={ariaLabel}
    >
      <span className='relative z-10 flex items-center gap-2'>
        {children}
        <FiExternalLink className='h-4 w-4 duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 motion-safe:transition-transform' />
      </span>
