...
export default function CompetitionInfo({ dict }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Main Content */}
      <div className='relative z-10 mx-auto max-w-screen-xl px-4 pt-8 pb-12 sm:pt-10 sm:pb-16 lg:pt-12 lg:pb-20'>
    ...
    <div className='space-y-6 lg:col-span-7'>
      ...
      <header>
        <h1
          ...
          className={clsx(
            'text-2xl font-extrabold tracking-wide text-sky-500 uppercase sm:text-3xl lg:text-4xl',
      ...
      {intro && (
        <div
          className={clsx(
            'delay-200 duration-1000 ease-out motion-safe:transition-all',
            isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
          )}
        >
      ...
      {ruleItems.length > 0 && (
        <div
          className={clsx(
            'delay-400 duration-1000 ease-out motion-safe:transition-all',
            isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
          )}
        >
      ...
      {noteItems.length > 0 && (
        <div
          className={clsx(
            'space-y-4 delay-600 duration-1000 ease-out motion-safe:transition-all',
            isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
          )}
        >
      ...
      <div
        className={clsx(
          'delay-800 duration-1000 ease-out motion-safe:transition-all',
          isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
        )}
      >
        <RegulationsButton />
...
function RuleItem({ children, index, isVisible }: RuleItemProps) {
  return (
    <li
      className={clsx(
        'relative text-sm leading-relaxed text-slate-800/90 duration-700 ease-out motion-safe:transition-all sm:text-base lg:text-lg',
        'before:absolute before:top-2 before:-left-5 before:h-1.5 before:w-1.5 before:rounded-full before:bg-sky-500',
        ...
        <div className='hidden sm:grid sm:grid-cols-2 sm:gap-10 lg:gap-16'>
          {sponsors.map((sponsor, index) => (
            <div
              key={sponsor.alt}
              className={clsx(
                'flex items-center justify-center duration-700 ease-out motion-safe:transition-all',
                isVisible
...
function RegulationsButton() {
  return (
    ...
    >
      <FiDownload
        className='h-4 w-4 duration-300 group-hover:scale-110 motion-safe:transition-transform'
        aria-hidden='true'
      />
