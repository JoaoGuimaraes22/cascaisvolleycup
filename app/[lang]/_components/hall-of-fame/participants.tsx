...
type Props = {
  dict: HallOfFameParticipantsDict
}
...
function TeamItem({ team, index, isVisible }: TeamItemProps) {
  ...
  return (
    <li
      className={clsx(
        'flex items-center gap-3 rounded-lg bg-white/60 px-3 py-2 shadow-sm ring-1 ring-black/5 backdrop-blur-sm duration-500 ease-out motion-safe:transition-all',
...
function PlayersImage({ src, alt, isVisible }: PlayersImageProps) {
  return (
    <div className='relative hidden lg:col-span-6 lg:block'>
      <div
        className={clsx(
          'relative z-10 mx-auto -mt-2 h-[340px] w-full overflow-visible duration-1000 ease-out motion-safe:transition-all sm:-mt-4 sm:h-[420px] lg:-mt-6 lg:h-[520px]',
...
function CTAButton({ onClick, children, isVisible }: CTAButtonProps) {
  return (
    <button
      ...
      onClick={onClick}
      className={clsx(
        'group relative mt-6 inline-flex items-center gap-2 overflow-hidden rounded-full bg-gradient-to-r from-sky-700 to-sky-800 px-6 py-3 text-sm font-bold text-white shadow-xl ring-1 ring-black/10 duration-300 hover:scale-105 hover:shadow-2xl focus-visible:ring-2 focus-visible:ring-sky-300 focus-visible:ring-offset-2 focus-visible:outline-none motion-safe:transition-all sm:text-base',
...
function WaveSection({ isVisible }: WaveSectionProps) {
  return (
    <div className='pointer-events-none absolute bottom-0 left-1/2 w-screen -translate-x-1/2'>
      <div
        className={clsx(
          'relative duration-1000 ease-out motion-safe:transition-all',
          isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
        )}
        style={{
...
export default function HallOfFameParticipants({ dict }: Props) {
          ...
          <div className='lg:col-span-6'>
            <header>
              <h2
                ...
                className={clsx(
                  'mb-4 text-xl font-extrabold tracking-wide text-sky-500 uppercase duration-1000 ease-out motion-safe:transition-all sm:text-2xl',
                  isVisible
            ...
            >
              ...
              {hasMoreTeams && !showAllTeams && (
                <button
                  ...
                  className={clsx(
                    'mt-4 inline-flex items-center gap-2 rounded-lg bg-white/70 px-4 py-2 text-sm font-semibold text-sky-700 shadow-sm ring-1 ring-black/5 backdrop-blur-sm duration-300 hover:bg-white/90 hover:shadow-md focus-visible:ring-2 focus-visible:ring-sky-300 focus-visible:outline-none motion-safe:transition-all sm:hidden',
                    'duration-700 ease-out motion-safe:transition-all',
                    isVisible
                      ? 'translate-y-0 opacity-100'
                      : 'translate-y-6 opacity-0'
